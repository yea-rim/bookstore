...
<%--                                <form action="${pageContext.request.contextPath }/pay/purchase" method = "get"> --%>
                            <tbody>
                         		<c:forEach var="StorePayVO" items="${basketStoreList.store }" varStatus="status">
                                     <input type = "hidden" name = "finalStore[${status.index }].storeBookNo" value = "${StorePayVO.storeNo }">
                                   <input type = "hidden" name = "finalStore[${status.index }].quantity" value = "${StorePayVO.quantity }">
                         			
                         		</c:forEach>
                         		<c:forEach var="UsedPayVO" items="${basketUsedList.used }" varStatus="status">
                                     <input type = "hidden" name = "finalusedStore[${status.index }].usedNo" value = "${UsedPayVO.usedNo }">
                                   <input type = "hidden" name = "finalusedStore[${status.index }].quantity" value = "${UsedPayVO.quantity }">
...
<%--                                 <input type = "hidden" name = "finalUsed[${status.index }].usedNo" value = "${usedDto.usedNo }"> --%>
                                <tr>
                                    <td class="shoping__cart__item">
                                        <img src="${basketDto.basketBookImage }">
                                    </td>
                                     <td  style = "text-align:left">
                              <h5>${basketDto.basketBookTitle }</h5>
                                     </td>
                                    <td class="shoping__cart__price">
                                      	
<%--                                        ${basketDto.basketPrice/basketDto.basketAmount }원                                   </td> --%>
                                    <td class="shoping__cart__quantity">
                                       <h5>${basketDto.basketAmount}개</h5>
<!--                                         <div class="quantity"> -->
<!--                                             <div class="pro-qty"> -->
